...
class BPMN:
    ...
    def add_path_to_workflow(self, path) -> list:
        ordered_keys = path["OrderedKeys"]
        return_paths = [[]]
        gateway_strings = []
        ...
        process_strings = []
        for key in path["OrderedKeys"]:
            if key.find("Processes") != -1:
                process_strings.append(key)

        for key in ordered_keys:
            if key in gateway_strings:
                current_paths = deepcopy(return_paths[:])
                current_paths_number = len(current_paths)
                for j in range(path[key]["PathNumbers"] - 1):
                    for k in range(len(current_paths)):
                        return_paths.append(current_paths[k])
                for j in range(path[key]["PathNumbers"]):
                    internal_return_path = self.add_path_to_workflow(path[key][f"Path{j + 1}"])
                    for l in range(j * current_paths_number, j * current_paths_number + current_paths_number):
                        for k in range(len(internal_return_path)):
                            return_paths[l].extend(internal_return_path[k])
            if key in process_strings:
                for j in range(len(return_paths)):
                    return_paths[j].extend(path[key][:])
        return return_paths
